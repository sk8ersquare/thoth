...
fn main() {
    ...
    // Windows: ONNX Runtime (embedded in sherpa-onnx static archive) requires
    // advapi32 for ETW telemetry symbols (EventWriteTransfer, EventRegister, etc.).
    // espeak-ng (also embedded) requires advapi32 for registry access
    // (RegOpenKeyExA, RegQueryValueExA). These are not linked automatically.
    if target_os == "windows" {
        link_lib("advapi32", true);
    }
